...
void main() {
    ...
    waterDepth = clamp(waterDepth/1.0, 0.0, 1.0);
    ...
    vec4 refractColor = texture(refractionTexture, vec2(refractTexCoords.x, refractTexCoords.y));
    FragColor = mix(reflectColor, refractColor, fresnelFactor);
    FragColor = mix(FragColor, vec4(0.0, 0.3, 0.5, 1.0) + vec4(specularLight, 0.0), 0.2);
